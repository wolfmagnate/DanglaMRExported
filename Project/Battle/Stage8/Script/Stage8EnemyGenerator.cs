...
public class Stage8EnemyGenerator : MonoBehaviour
{
    void Start()
    {
    ...
    private IEnumerator GenEnemy()
    {
        ...
        CreateEnemy(enemys[2], forwordPosition(6, -120), effects[2]);
        yield return new WaitForSeconds(12);
        CreateEnemy(enemys[3], forwordPosition(6, 20) + Vector3.up, effects[3]);
        CreateEnemy(enemys[3], forwordPosition(6, 0), effects[3]);
        CreateEnemy(enemys[3], forwordPosition(6, -20) + Vector3.down, effects[3]);
        yield return new WaitForSeconds(12);
        CreateEnemy(enemys[0], forwordPosition(6, 0) + Vector3.up, effects[0]);
        CreateEnemy(enemys[0], forwordPosition(6, 0), effects[0]);
        CreateEnemy(enemys[0], forwordPosition(6, 0) + Vector3.down, effects[0]);
    ...
    private GameObject CreateEnemy(GameObject enemy, Vector3 position, GameObject effect)
    {
        ...
        generatedEnemy.GetComponent<EnemyStatusController>().OnDie.AddListener(() => GameObject.Find("Manager").GetComponent<Stage8SceneControl>().AddKillCounter());
